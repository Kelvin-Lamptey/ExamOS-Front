...
test.beforeEach(async () => {
  ...
  service = await createMockService({ dataDir, offline: true, syncDelayMs: 100 })
...
async function login(page: Page) {
  ...
  await expect(page.getByRole('heading', { name: 'Welcome back, Kelvin.' })).toBeVisible()
}
async function start(page: Page) {
  ...
  await expect(page.getByRole('heading', { name: /Which object-oriented concept/ })).toBeVisible()
}
const answer = (page: Page) => page.getByLabel('Your answer', { exact: true })

test('complete all five question types offline, restore, submit once, and sync', async ({ page }) => {
  ...
  page.on('request', request => {
    if (/^https?:/.test(request.url()) && !request.url().startsWith('http://127.0.0.1:')) external.push(request.url())
    if (request.method() === 'PUT') revisions.push(request.postDataJSON().revision)
    if (request.method() === 'POST' && request.url().endsWith('/submit')) submits++
  })
  await login(page)
  ...
  await page.getByLabel('Rough work').fill('Home notes, isolated from exam notes.')
  await page.getByRole('button', { name: 'Close dialog' }).click()
  await start(page)
  await page.getByRole('button', { name: /scratchpad/i }).click()
  ...
  await page.screenshot({ path: 'test-results/exam-runner.png', fullPage: true })
  await page.getByRole('button', { name: 'Next question' }).click()
  await answer(page).fill('Use case diagram')
  await page.getByRole('button', { name: 'Next question' }).click()
  await answer(page).fill('Cohesion groups related responsibilities.\nCoupling describes dependencies between classes.')
  await page.getByRole('button', { name: 'Next question' }).click()
  await page.getByLabel('Your numeric answer').fill('42.5')
  await page.getByRole('button', { name: 'Next question' }).click()
  await page.getByLabel('Your code', { exact: true }).fill('public class Student {\n  private String name;\n}')
  await expect(page.getByText('Saved locally – waiting to sync', { exact: true })).toBeVisible()
  await page.reload()
  await expect(page.getByRole('radio', { name: 'Encapsulation', exact: true })).toBeChecked()
  ...
  await page.getByRole('button', { name: 'Question 3, answered', exact: true }).click()
  await expect(answer(page)).toHaveValue('Cohesion groups related responsibilities.\nCoupling describes dependencies between classes.')
  await page.getByRole('button', { name: 'Question 4, answered', exact: true }).click()
  await expect(page.getByLabel('Your numeric answer')).toHaveValue('42.5')
  await page.getByRole('button', { name: 'Question 5, answered', exact: true }).click()
  await expect(page.getByLabel('Your code', { exact: true })).toHaveValue('public class Student {\n  private String name;\n}')
  await page.getByRole('button', { name: 'Review & submit' }).click()
  await expect(page.getByRole('dialog')).toContainText('5 of 5 questions answered')
  await page.getByRole('button', { name: 'Submit final answers' }).dblclick()
  await expect(page.getByRole('heading', { name: 'Your exam is submitted.' })).toBeVisible()
...
test('recoverable boot, exact access code, and small-screen layout', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Welcome back, Kelvin.' })).toBeVisible()
  await page.screenshot({ path: 'test-results/mobile-launcher.png', fullPage: true })
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  await start(page)
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
})

test('failed local save prevents navigation and preserves the draft for retry', async ({ page }) => {
  await login(page); await start(page)
  await page.getByRole('button', { name: 'Next question' }).click()
  ...
  await expect(page.getByText('Not saved locally', { exact: true })).toBeVisible()
...
test('a lost submit response is reconciled from the backend without a second POST', async ({ page }) => {
  await login(page); await start(page)
  let submissions = 0
...
test('expired sessions restore the same student without losing unsaved typing', async ({ page }) => {
  await login(page); await start(page)
  await page.getByRole('button', { name: 'Next question' }).click()
  await page.route('**/v1/exams/*/answers/*', route => route.fulfill({ status: 401, contentType: 'application/json', body: JSON.stringify({ error: { code: 'SESSION_EXPIRED', message: 'Sign in again.', retryable: false } }) }), { times: 1 })
  await answer(page).fill('Keep my draft during reauthentication')
  await expect(page.getByRole('dialog', { name: 'Restore your student session' })).toBeVisible()
  await expect(page.getByLabel('Student ID', { exact: true })).toHaveValue('GCTU-CS-001')
  await page.getByLabel('Access code', { exact: true }).fill('A7K2')
  await page.getByRole('button', { name: 'Enter workspace' }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  await expect(answer(page)).toHaveValue('Keep my draft during reauthentication')
  await expect(page.getByText('Saved locally – waiting to sync', { exact: true })).toBeVisible()
})

test('exam navigation flushes a draft before leaving the runner', async ({ page }) => {
  await login(page); await start(page)
  await page.getByRole('button', { name: 'Next question' }).click()
...
test('deadline expiry submits automatically once and leaves the exam locked', async ({ page }) => {
  await login(page)
  let deadline = ''
  let submits = 0
  page.on('request', request => { if (request.method() === 'POST' && request.url().endsWith('/submit')) submits++ })
