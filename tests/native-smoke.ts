...
const service = await createMockService({ dataDir: join(temporary, 'mock'), offline: true })
...
const executable = resolve(process.env.EXAMOS_NATIVE_BINARY ?? 'src-tauri/target/release/exam-os')
const child = spawn('xvfb-run', ['-a', executable], {
  detached: true,
  stdio: ['ignore', 'pipe', 'pipe'],
  env: { ...process.env, XDG_DATA_HOME: join(temporary, 'data'), XDG_CONFIG_HOME: join(temporary, 'config'), WEBKIT_DISABLE_COMPOSITING_MODE: '1' },
})
...
try {
  await Promise.race([
    booted,
    new Promise<never>((_, reject) => { timeout = setTimeout(() => reject(new Error(`Native boot did not reach the local session endpoint.\n${output}`)), 20_000) }),
    exited.then(() => { throw new Error(`Native process exited before boot.\n${output}`) }),
  ])
  console.log(`Native WebKit boot passed. Origin: ${origin}. Requests: ${[...paths].join(', ')}`)
} finally {
