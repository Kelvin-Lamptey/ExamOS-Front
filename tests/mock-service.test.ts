...
describe('persistent local mock API', () => {
  ...
  const answer = (revision: number, value = 'A saved answer') => ({
    revision,
    response: { type: 'text', value },
    client_saved_at: new Date(clock).toISOString(),
  })
  ...
  beforeEach(async () => {
    ...
    const response = await request(
      '/auth/login',
      'POST',
      { student_id: 'GCTU-CS-001', access_code: 'A7K2' },
      '',
    )
  ...
  it('persists all five answer shapes offline and reconstructs them after service restart', async () => {
    const initial = ExamPackageSchema.parse(
      await (await request('/exams/exam_ooad_001')).json(),
    )
    expect(initial.attempt).toBeNull()
    const started = ExamPackageSchema.parse(
      await (await request('/exams/exam_ooad_001/start', 'POST', {})).json(),
    )
    expect(started.attempt?.expires_at).toBe('2026-09-05T12:00:00.000Z')
    const responses = [
      { type: 'mcq', selected_option_ids: ['a'] },
      { type: 'text', value: 'Short answer' },
    ...
    for (const [i, response] of responses.entries()) {
      const result = await request(
        `/exams/exam_ooad_001/answers/q${i + 1}`,
        'PUT',
        {
          revision: i + 1,
          response,
    ...
    await service.close()
    await boot()
    const restored = ExamPackageSchema.parse(
      await (await request('/exams/exam_ooad_001')).json(),
    )
    expect(restored.answers.map((a) => a.response)).toEqual(responses)
    expect(restored.attempt).toEqual({
    ...
    expect(
      (
        await request('/exams/exam_ooad_001/answers/q4', 'PUT', {
          ...answer(6),
          response: { type: 'number', value: null },
        })
      ).status,
    ).toBe(200)
  })
  ...
  it('serializes revisions and makes an identical retry idempotent', async () => {
    ...
    expect(
      (await request('/exams/exam_ooad_001/answers/q2', 'PUT', answer(7)))
        .status,
    ).toBe(200)
    expect(
      (await request('/exams/exam_ooad_001/answers/q2', 'PUT', answer(7)))
    ...
    const stale = await request(
      '/exams/exam_ooad_001/answers/q2',
      'PUT',
      answer(6),
    )
    expect(stale.status).toBe(409)
    expect(await stale.json()).toMatchObject({
      error: { code: 'STALE_REVISION', current_revision: 7 },
    })
    expect(
      (
        await request(
    ...
    expect(
      (await request('/exams/exam_ooad_001/answers/q3', 'PUT', answer(7)))
        .status,
    ).toBe(409)
  })
  ...
  it('permanently locks an idempotent submission, including after restart', async () => {
    ...
    expect(
      (
        await request('/exams/exam_ooad_001/submit', 'POST', {
    ...
    const receipt = await (
      await request('/exams/exam_ooad_001/submit', 'POST', submission)
    ).json()
    ...
    await boot()
    expect(
      (await request('/exams/exam_ooad_001/answers/q2', 'PUT', answer(2)))
        .status,
    ).toBe(409)
    expect(
      ExamPackageSchema.parse(
        await (await request('/exams/exam_ooad_001')).json(),
      ).submission,
    ).toEqual(receipt)
  })
  ...
  it('isolates student answers and does not start unavailable exams', async () => {
    expect((await request('/exams/exam_db_002/start', 'POST', {})).status).toBe(
      403,
    )
    expect(
      (await request('/exams/exam_networks_003/start', 'POST', {})).status,
    ).toBe(403)
    await request('/exams/exam_ooad_001/start', 'POST', {})
    await request('/exams/exam_ooad_001/answers/q2', 'PUT', answer(1))
    ...
    const exam = ExamPackageSchema.parse(
      await (
        await request(
          '/exams/exam_ooad_001',
          'GET',
  ...
  it('syncs locally queued answers after simulated Internet reconnects', async () => {
    await request('/exams/exam_ooad_001/start', 'POST', {})
    await request('/exams/exam_ooad_001/answers/q2', 'PUT', answer(1))
    expect(await (await request('/system/status')).json()).toMatchObject({
    ...
    await expect
      .poll(
        async () =>
          (await (await request('/system/status')).json()).pending_count,
      )
      .toBe(0)
    expect(
      ExamPackageSchema.parse(
        await (await request('/exams/exam_ooad_001')).json(),
      ).answers[0]?.sync_state,
    ).toBe('synced')
  })
})
