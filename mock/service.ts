...
export async function createMockService(options: MockOptions) {
  ...
  function getRecord(studentId: string, examId: string): ExamRecord {
    return (
      state.records[studentId]?.[examId] ?? {
        attempt: null,
        answers: {},
        submission: null,
      }
  ...
  function getPackage(exam: SeedExam, studentId: string): ExamPackage {
    ...
    return {
      ...exam,
      status: status(exam, record),
      server_time: isoNow(),
      answers: Object.values(record.answers),
  ...
  function systemStatus(studentId: string): SystemStatus {
    ...
    const pending = records
      ...
      .filter((item) => item.sync_state !== 'synced').length
    return {
      connectivity: offline ? 'offline' : 'online',
      sync_state: offline ? 'offline' : pending ? 'queued' : 'synced',
    ...
    for await (const chunk of req) {
      ...
      if (length > 512_000)
        throw new HttpError(
          422,
          'PAYLOAD_TOO_LARGE',
          'This answer exceeds the local service size limit.',
        )
  ...
  async function route(
    ...
    if (req.method === 'GET' && path === '/v1/health')
      return { data: { status: 'ok', contract_version: 'v1', mode: 'mock' } }
    if (req.method === 'POST' && path === '/v1/auth/login') {
      const credentials = LoginRequestSchema.parse(await body(req))
      const person = students.find(
        (s) =>
          s.student.student_id === credentials.student_id &&
          s.accessCode === credentials.access_code,
      )
      if (!person)
        throw new HttpError(
          401,
          'INVALID_CREDENTIALS',
          'Student ID or access code is invalid.',
        )
    ...
    if (req.method === 'GET' && path === '/v1/system/status')
      return { data: systemStatus(studentId) }
    if (req.method === 'POST' && path === '/v1/auth/logout') {
      if (
        Object.values(state.records[studentId] ?? {}).some(
          (r) => r.attempt && !r.submission?.local_locked,
        )
      ) {
        throw new HttpError(
          409,
          'EXAM_IN_PROGRESS',
          'Submit your in-progress exams before signing out.',
        )
      }
      const next = structuredClone(state)
      ...
      return {
        data: {
          exams: state.exams.map((exam) => ({
            ...
            allowed_utilities: exam.allowed_utilities,
    ...
    if (!exam)
      throw new HttpError(404, 'EXAM_NOT_FOUND', 'This exam does not exist.')
    const action = match[2]
    const record = structuredClone(getRecord(studentId, examId))
    if (req.method === 'GET' && !action)
      return { data: getPackage(exam, studentId) }
    if (req.method === 'POST' && action === 'start') {
      if (record.submission?.local_locked || record.attempt)
        return { data: getPackage(exam, studentId) }
      if (status(exam, record) !== 'available')
        throw new HttpError(
          403,
          'EXAM_UNAVAILABLE',
      ...
      record.attempt = {
        ...
        expires_at: new Date(
          Math.min(
            now() + exam.duration_minutes * 60_000,
    ...
    if (req.method === 'PUT' && action === 'answers' && match[3]) {
      if (record.submission?.local_locked)
        throw new HttpError(
          409,
          'EXAM_LOCKED',
          'This exam has already been submitted. Your answers are locked.',
        )
      if (!record.attempt)
        throw new HttpError(
          409,
          'EXAM_NOT_STARTED',
          'Start this exam before saving answers.',
        )
      ...
      if (
        ...
        JSON.stringify(previous.response) === JSON.stringify(input.response)
      ) {
        const { response: _response, ...ack } = previous
        return { data: ack }
      }
      if (now() > Date.parse(record.attempt.expires_at) + 30_000)
        throw new HttpError(
          409,
          'TIME_EXPIRED',
          'The exam time has ended. Ask your invigilator about any unconfirmed changes.',
        )
      if (input.revision <= record.attempt.last_revision)
        throw new HttpError(
          409,
          'STALE_REVISION',
          'A newer answer revision is already saved. Reload the exam state before retrying.',
          false,
          record.attempt.last_revision,
        )
      ...
      if (response.type !== expectedType)
        throw new HttpError(
          ...
          'This answer does not match the question type.',
        )
      if (
        response.type === 'mcq' &&
        response.selected_option_ids.some(
      ...
      if (
        response.type === 'code' &&
        response.language !== (question.code_config?.language ?? 'text')
      )
        throw new HttpError(
          422,
          'INVALID_LANGUAGE',
          'Use the language supplied with this question.',
        )
      const saved = {
        question_id: questionId,
        revision: input.revision,
        local_saved: true,
        local_saved_at: isoNow(),
        sync_state: offline ? ('offline' as const) : ('queued' as const),
        response,
      }
    ...
    if (req.method === 'POST' && action === 'submit') {
      ...
      if (record.submission) {
        if (record.submission.submission_id === input.submission_id)
          return { data: record.submission }
        throw new HttpError(
          409,
          'EXAM_LOCKED',
          'This exam has already been submitted. Check its submission status.',
        )
      }
      if (!record.attempt)
        throw new HttpError(
          409,
          'EXAM_NOT_STARTED',
          'Start this exam before submitting.',
        )
      if (record.attempt.last_revision !== input.final_revision)
        throw new HttpError(
          409,
          'REVISION_MISMATCH',
          'The final revision does not match locally saved answers. Review the saved state before submitting.',
          false,
          record.attempt.last_revision,
        )
      record.submission = {
        ...
        state: 'submitted',
        local_locked: true,
        sync_state: offline ? 'offline' : 'queued',
      }
      await updateRecord(studentId, examId, record)
      return { data: record.submission }
    }
