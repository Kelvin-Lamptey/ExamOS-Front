...
export function calculate(expression: string): string {
  ...
  if (!source || source.length > 200) throw new Error('Enter an expression of up to 200 characters.')
  ...
  function factor(): number {
    ...
    if (token === '(') {
      value = sum()
      if (tokens![position++] !== ')') throw new Error('Close the parentheses to finish this expression.')
  ...
  const result = sum()
  if (position !== tokens.length) throw new Error('Check the operators in your expression.')
  if (!Number.isFinite(result)) throw new Error('The result is too large to display.')
