  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new ApiError(
        ...
        error?.message ??
          `The local service returned an error (${response.status}). Please try again.`,
        response.status,
    ...
    if (!result.success) {
      throw new ApiError(
        'CONTRACT_MISMATCH',
        'The local service returned an incompatible response. Ask your invigilator to check the service version.',
      )
    }
    return result.data
  } catch (error) {
    ...
    throw new ApiError(
      'SERVICE_UNAVAILABLE',
      'The local Exam OS service is unavailable. Your latest changes have not been confirmed. Reconnect to the local service and retry.',
      0,
      true,
    )
  } finally {
...
export const api = {
  ...
  exams: async () =>
    (await request('/exams', z.object({ exams: z.array(ExamSummarySchema) })))
      .exams,
  exam: (id: string) => request(examPath(id), ExamPackageSchema),
  startExam: (id: string) =>
    request(`${examPath(id)}/start`, ExamPackageSchema, {
      method: 'POST',
      body: {},
    }),
  saveAnswer: (
    ...
    request(
      `${examPath(examId)}/answers/${encodeURIComponent(questionId)}`,
      AnswerAcknowledgementSchema,
      { method: 'PUT', body, keepalive },
    ),
  systemStatus: () => request('/system/status', SystemStatusSchema),
  submit: (id: string, body: SubmissionRequest) =>
    request(`${examPath(id)}/submit`, SubmissionSchema, {
      method: 'POST',
      body,
    }),
}
