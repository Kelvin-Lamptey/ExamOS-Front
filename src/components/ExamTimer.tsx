...
export function ExamTimer({
  ...
  return (
    <div className={`exam-timer ${remaining <= 300 ? 'timer-warning' : ''}`}>
      <Clock3 className="size-4" />
      <div>
        <p
          className="font-mono text-lg font-medium leading-tight tabular-nums"
          aria-label={`${hours} hours ${minutes} minutes ${seconds} seconds remaining`}
        >
          {time}
        </p>
        <p className="mt-1 text-[9px] tracking-wider uppercase">
          Time remaining
        </p>
      </div>
      {remaining <= 300 && (
        <span className="sr-only" role="status">
          {remaining === 0
            ? 'Time has ended. Submitting your saved answers.'
