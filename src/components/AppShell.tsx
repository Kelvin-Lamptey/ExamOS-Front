    ...
    <div className={inRunner ? 'app-shell runner-shell' : 'app-shell'}>
      ...
      <header className="app-header">
        ...
        <div className="hidden items-center gap-2 text-xs text-muted md:flex">
          ...
          Student workspace
          {health.mode === 'mock' && (
            <span className="ml-2 rounded border border-line px-2 py-0.5 font-mono text-[10px] text-accent">
              MOCK
            </span>
          )}
        </div>
        <div className="ml-auto flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div
              className="grid size-9 place-items-center rounded-full border border-accent/20 bg-accent/10 text-xs font-medium text-accent"
        ...
        <aside className="workspace-sidebar">
          ...
          <nav className="mt-4 space-y-1" aria-label="Main navigation">
            <NavLink to="/" end className="sidebar-link">
              ...
              My exams<span className="ml-auto text-accent">↗</span>
            </NavLink>
          </nav>
          <div className="mt-10 border-t border-line px-3 pt-6">
            <BookOpen className="mb-3 size-5 text-accent" />
            <p className="text-sm font-medium">One thing at a time.</p>
            <p className="mt-2 text-xs leading-relaxed text-muted">
              Settle in. Take a breath.
              <br />
              You’ve prepared for this.
            </p>
          </div>
          <div className="mt-auto space-y-5 px-3 pt-8">
            ...
            <div className="flex items-center gap-2 border-t border-line pt-5 text-[10px] text-muted">
              <ShieldCheck className="size-3.5" />
              Powered by KenolTech
            </div>
          </div>
        </aside>
      )}
      <main
        id="main-content"
        className={inRunner ? 'runner-main' : 'workspace-main'}
        tabIndex={-1}
      >
        <Outlet />
      </main>
