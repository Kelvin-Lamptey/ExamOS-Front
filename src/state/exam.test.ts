...
function fixture(): ExamPackage {
  return {
    ...createExams(Date.parse('2026-09-05T10:00:00Z'))[0]!,
    status: 'in_progress',
    server_time: '2026-09-05T10:00:00Z',
    answers: [],
    attempt: {
...
function acknowledgement(
  ...
  return {
    ...
    local_saved: true,
    local_saved_at: input.client_saved_at,
    sync_state: 'queued',
  }
}
function harness(exam = fixture()) {
  const backend = {
    saveAnswer: vi.fn(
      async (_examId: string, id: string, input: AnswerSaveRequest) =>
        acknowledgement(id, input),
    ),
...
describe('exam persistence and submission invariants', () => {
  it('debounces typing and flushes immediately for navigation', async () => {
    ...
    expect(backend.saveAnswer.mock.calls[0]?.[2]).toMatchObject({
      revision: 1,
  ...
  it('preserves typing during an in-flight save and serializes subsequent revisions', async () => {
    ...
    backend.saveAnswer.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
          complete = resolve
        }),
    ...
    await flushing
    expect(
      backend.saveAnswer.mock.calls.map((call) => call[2].revision),
    ).toEqual([1, 2, 3])
    expect(controller.getSnapshot().answers.q2?.response).toEqual({
      type: 'text',
      value: 'new while saving',
    })
    expect(controller.getSnapshot().dirty).toBe(false)
    controller.dispose()
  })
  ...
  it('never treats local_saved=false as successful and retries the exact revision', async () => {
    const { controller, backend } = harness()
    backend.saveAnswer.mockImplementationOnce(async (_examId, id, input) => ({
      ...acknowledgement(id, input),
      local_saved: false,
    }))
  ...
  it('starts above recovered revisions and preserves a draft through stale-write reconciliation', async () => {
    ...
    backend.saveAnswer.mockRejectedValueOnce(
      new ApiError('STALE_REVISION', 'Stale revision', 409, false, 20),
    )
    backend.exam.mockResolvedValueOnce({
      ...exam,
      attempt: { ...exam.attempt!, last_revision: 20 },
    })
  ...
  it('does not submit if a save fails, and coalesces double-click submissions after flush', async () => {
    const { controller, backend } = harness()
    backend.saveAnswer.mockRejectedValueOnce(
      new ApiError('SERVICE_UNAVAILABLE', 'Offline local service', 0, true),
    )
    ...
    expect(backend.submit.mock.calls[0]?.[1]).toMatchObject({
      final_revision: 1,
    })
  ...
  it('reconciles a lost submission response without issuing another POST', async () => {
    ...
    backend.submit.mockImplementationOnce(async (_id, input) => {
      backend.exam.mockResolvedValue({
        ...fixture(),
        submission: {
          exam_id: fixture().id,
          submission_id: input.submission_id,
  ...
  it('freezes an uncertain outcome and reuses the submission ID on an explicit retry', async () => {
    const { controller, backend } = harness()
    backend.submit.mockRejectedValueOnce(
      new ApiError('SERVICE_UNAVAILABLE', 'Lost response'),
    )
    ...
    expect(backend.submit.mock.calls[0]?.[1].submission_id).toBe(
      backend.submit.mock.calls[1]?.[1].submission_id,
    )
  })
  ...
  it('distinguishes zero, cleared numbers and invalid unfinished numeric input', async () => {
    ...
    controller.change(
      ...
      { raw: '-', error: 'Finish entering a valid number.' },
    )
    await expect(controller.flush()).rejects.toMatchObject({
      code: 'INVALID_ANSWER',
    })
