...
export function AppRoot() {
      ...
      <main className="boot-screen">
        ...
        <div className="max-w-md text-center">
          ...
          <p className="mt-4 leading-relaxed text-muted">
            {health.isError
              ? 'The local Exam OS service isn’t responding yet. We’ll keep checking. You can retry, or ask your invigilator for help.'
              : 'Connecting to the local Exam OS service. You’ll be ready in a moment.'}
          </p>
          {health.isError ? (
            <button
            ...
            <LoaderCircle className="mx-auto mt-7 size-6 animate-spin text-accent" />
          )}
          {health.isError && (
            <p className="mt-5 text-xs text-muted">
              This is a local service connection. Internet access is not
              required.
            </p>
          )}
        </div>
  ...
  return (
    <AppContext.Provider value={{ session: session.data, health: health.data }}>
      <Outlet />
      {reauth && session.data && (
        <Modal title="Restore your student session">
          <p className="mb-6 text-sm leading-relaxed text-muted">
            Your session has expired. Sign in with the same student ID to
            continue. Your current work stays on the page.
          </p>
          <LoginForm
            studentId={session.data.student.student_id}
            onSuccess={(restored) => {
              queryClient.setQueryData(['session'], restored)
              setReauth(false)
              void queryClient.invalidateQueries({
                predicate: (query) =>
                  !['session', 'health'].includes(String(query.queryKey[0])),
              })
              window.dispatchEvent(new Event('examos:session-restored'))
            }}
          />
        </Modal>
      )}
